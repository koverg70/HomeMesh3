...
#ifdef WWW_client
	// ----- http get
void EtherShield::ES_client_browse_url(const char *urlbuf, char *urlbuf_varpart, const char *hoststr,void (*callback)(uint8_t,uint16_t)) {
	client_browse_url(urlbuf, urlbuf_varpart, hoststr,callback);
}

void EtherShield::ES_client_http_post(const char *urlbuf, const char *hoststr, const char *additionalheaderline, const char *method, char *postval,void (*callback)(uint8_t,uint16_t)) {
